...
def calculate_defensive_calibrator(
    ...
    # 2. Track Raid Damage Taken Per Second (DTPS)
    ...
    for event in events:
        ...
        if 0 <= elapsed < len(raid_dtps):
            ...
            # Track which spell contributed the hit
            spell_name = "Unknown Boss Attack"
            ability = event.get("ability")
            if isinstance(ability, dict):
                spell_name = ability.get("name") or spell_name
            elif isinstance(ability, str):
                spell_name = ability
            else:
                spell_name = event.get("abilityName") or event.get("name") or spell_name
    ...
    # 3. Track All Major Defensive Casts
    defensive_casts = []
    for event in events:
    ...
    # 4. Scan for Major Damage Spikes
    spikes = []
    avg_dtps = sum(raid_dtps) / max(1.0, duration_s)
    
    # Threshold is adaptive: at least 3x the average DTPS, and a minimum baseline of 400k raw damage
    spike_threshold = max(3.0 * avg_dtps, 400000.0)

    # Prevent multiple rolling ticks in a row from counting as individual spikes by using a cooldown window (4 seconds)
    last_spike_seconds = -10

    for t in range(1, len(raid_dtps) - 1):
        # 2-second rolling damage total
        rolling_damage = raid_dtps[t] + raid_dtps[t - 1]

        if rolling_damage >= spike_threshold:
            if t - last_spike_seconds >= 4:
